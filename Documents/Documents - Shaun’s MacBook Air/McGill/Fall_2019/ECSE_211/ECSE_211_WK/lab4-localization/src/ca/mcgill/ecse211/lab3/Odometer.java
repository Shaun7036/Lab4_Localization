...
public class Odometer implements Runnable {
  ...
  public void run() {
    ...
    while (true) {
      ...
      // get the value after the reset and use it in calculation as basepoint
      double dL= (WHEEL_RAD * Math.PI * (leftMotorTachoCount - lastleftMotorTachoCount))/180;
      double dR= (WHEEL_RAD * Math.PI * (rightMotorTachoCount-lastrightMotorTachoCount) )/180;
      lastleftMotorTachoCount=leftMotorTachoCount;
      lastrightMotorTachoCount =rightMotorTachoCount;
      double d= dL-dR;
      double dT= d / TRACK ;//angle in radiant
      //dT=Math.toRadians(dT);
      double dH= (dL+dR)/2;//magnitude displacement
      double dx=dH*Math.sin(Math.toRadians(theta)+dT);//dT
      double dy=dH*Math.cos(Math.toRadians(theta)+dT);//dT
      odo.update(dx,dy,Math.toDegrees(dT));
